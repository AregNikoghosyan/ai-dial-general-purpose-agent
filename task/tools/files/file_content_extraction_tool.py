...
class FileContentExtractionTool(BaseTool):
    ...
    def description(self) -> str:
        return (
            "Extracts text content from an attached file. "
            "Supported formats: PDF (text only), TXT, CSV (returned as markdown table), HTML/HTM. "
            "For large files (>10,000 characters) pagination is enabled — the response ends with "
            "'**Page #X. Total pages: Y**' to signal that more content exists. "
            "Start with page=1 (default). If the file has multiple pages consider switching to "
            "the RAG search tool for more efficient retrieval instead of reading all pages."
        )
